...
cv::Mat LaneDetector::edgeDetector(cv::Mat img_noise) {
	...
	//equalizeHist( img_noise, output ); // histogram
	
	// Binarize gray image将灰度图二值化
	//cv::threshold(output, output, 140, 255, cv::cv::imshow("binary image", output);THRESH_BINARY);
...
*/
	...
	cv::Point pts[4] = {
		...
		cv::Point(0.42*col, 0.65*row),
		cv::Point(0.625*col, 0.65*row),
